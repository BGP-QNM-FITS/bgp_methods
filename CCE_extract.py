...
This script processes the SXS CCE data into a format suitable for qnm fits to the strain. 

1. Create an ABD object from the sxs h5 files.
2. abd.t gets the time array. 2.0 * abd.sigma.bar.dot is the Moreschi-Boyle convention for the strain. 
   the dot takes the time derivative hence getting the news(?). MF_to_WM changes from a ModesTimeSeries object 
   to a scri or sxs WaveForm Modes object. Then it takes the argmax of the norm, i.e. the max value of the 
   news and shifts the time array so that the peak luminosity is at t=0.
3. Then there's some kind of interpolation to the merger/ringdown regime. Need to determine specifically what fn does. 
...
for rssname, radii in zip(rssnames, rss):
    for fileindex, filenum in enumerate(filenums):
        for lev in levs:
            ...
            print("Level: ", lev, "Radius name: ", rssname, "Radius: ", radius, "File: ", filenum)
            ...
            abd = scri.SpEC.file_io.create_abd_from_h5(
                ...
                Psi4=f"{filepathraw}:rMPsi4_BondiCce_R{radius}.h5",
                Psi3=f"{filepathraw}:r2Psi3_BondiCce_R{radius}.h5",
                Psi2=f"{filepathraw}:r3Psi2OverM_BondiCce_R{radius}.h5",
                Psi1=f"{filepathraw}:r4Psi1OverM2_BondiCce_R{radius}.h5",
                Psi0=f"{filepathraw}:r5Psi0OverM3_BondiCce_R{radius}.h5",
                file_format="RPDMB",
            )
